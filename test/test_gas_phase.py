...
from library.gas_phase.gas_phase_species import GasPhaseSpecies
...
def test_heat_capacity(gas_phase):
    ...
    q1 = sum(species.concentration * species.heat_capacity *
             gas_phase.temperature for species in gas_phase.species.values())
    ...
    for T in temperatures:
        ...
        for species_name in gas_phase.species:
            ...
            if T == 300:  # Verify against expected values at 300K
                print(
                    f"Expected at 300K: {expected_values[species_name]:.3f} J/(mol·K)"
                )
...
def test_standard_enthalpy(gas_phase):
    ...
    for T in temperatures:
        ...
        for species_name in gas_phase.species:
            ...
            # Calculate expected value using analytical formula
            coeffs = gas_phase._CP_COEFFICIENTS[species_name]
            H_298 = gas_phase._FORMATION_ENTHALPIES[species_name][
                'H_298'] * 1000  # kJ to J

            # Calculate H°(T)/R without a₆
            H_over_R = sum(coeff * T**(power + 1) / (power + 1)
                           for power, coeff in enumerate(coeffs))

            # Calculate H°(298)/R without a₆
            H_298_over_R = sum(coeff * 298.15**(power + 1) / (power + 1)
                               for power, coeff in enumerate(coeffs))

            # Solve for a₆ using H°₂₉₈
            a6 = (H_298 - gas_phase.R * H_298_over_R) / gas_phase.R

            # Calculate expected H°
            expected_H = gas_phase.R * (H_over_R + a6)
...
def test_standard_entropy(gas_phase):
    ...
    for T in temperatures:
        ...
        for species_name in gas_phase.species:
            ...
            # Calculate expected value using analytical formula
            coeffs = gas_phase._CP_COEFFICIENTS[species_name]
            S_298 = gas_phase._STANDARD_ENTROPIES[species_name]['S_298']

            # Calculate S°(T)/R without a₇
            S_over_R = coeffs[0] * log(T)  # First term
            S_over_R += sum(coeff * T**power / power
                            for power, coeff in enumerate(coeffs[1:], 1))

            # Calculate S°(298)/R without a₇
            S_298_over_R = coeffs[0] * log(298.15)
            S_298_over_R += sum(coeff * 298.15**power / power
                                for power, coeff in enumerate(coeffs[1:], 1))

            # Solve for a₇ using S°₂₉₈
            a7 = (S_298 - gas_phase.R * S_298_over_R) / gas_phase.R

            # Calculate expected S°
            expected_S = gas_phase.R * (S_over_R + a7)
